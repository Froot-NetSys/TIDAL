...
SEQ_CHUNK_LENGTH = [1024, 512, 256, 128, 64]
LOAD_GRAPH = True
...
def seq_chunk(seq):
    size = random.choice(SEQ_CHUNK_LENGTH)
    result = []
...
def resample(semi_data_path):
    ...
    non_sampling_path = semi_data_path + 'resampling/nonsampling.json'
    ...
    resampling_out = open( semi_data_path + 'resampling/FP_75_vary_train.json' ,'w' )
...
def main():
    ...
    os.makedirs( semi_data_path + 'resampling/' ,exist_ok=True )

    # Prepare the true-label data
    for file in os.listdir( semi_data_path ):
        if file.startswith( "train_data" ):
            print( "-------Find training file: " + file )
            ...
            if FP_ratio > 0:
                number_flip = int(y_train_all.count(1) * FP_ratio)
                count_i = 0
                for x in range(len(y_train_all)):
                    if count_i <= number_flip:
                        if y_train_all[x] == 0:
                            y_train_all[x] = 1
                            count_i += 1
                    else:
    ...
    nonsampling_out = open( semi_data_path + "resampling/vary_test.json" ,'w' )
    ...
    nonsampling_out = open( semi_data_path + "resampling/nonsampling.json" ,'w' )
